...
**驗證方式的限制**：這次改完後 Playwright 瀏覽器又被佔用（本機測試 server 還在 port 8934），只做了語法檢查（`new Function()` parse）＋ curl 確認 HTTP 200＋JSON 欄位結構比對，**沒有做到這次改動後的實際瀏覽器截圖驗證**。跟前一輪（合併前兩頁分開時）已經做過的視覺驗證相比，這次是新增功能（自動更新/展開狀態保留/警示邏輯），理論上風險更高一點，**建議使用者實際打開網站看一次，尤其展開族群卡片後等超過60秒看會不會被自動刷新打斷體驗**，如果有問題直接回報。
